...
class ActiveTeacherTrainer(DefaultTrainer):
        ...
        if proposal_type == "rpn":
            ...
            if hasattr(proposal_bbox_inst, "pred_masks"):
                ...
                bitmasks = roi_masks.to_bitmasks(
                    new_proposal_inst.gt_boxes,
                    image_shape[0],
                    image_shape[1],
                    mask_threshold,
                )
                polygons_masks = [
                    self.mask_to_polygons(bitmask)[0]
                    for bitmask in bitmasks.tensor.cpu().numpy()
                ]
    ...
    def run_step_full_semisup(self):
        ...
        assert (
            self.model.training
        ), "[ActiveTeacherTrainer] model was changed to eval mode!"
            ...
            for key in record_dict.keys():
                if key[:4] == "loss":
                    if key == "loss_rpn_loc_pseudo" or key == "loss_box_reg_pseudo":
                        ...
                        # loss_dict[key] = record_dict[key] * 0
                        loss_dict[key] = record_dict[key] * adjust_pseudo_weight(
                            self.iter, self.max_iter, 1, method="cosine"
                        )
        ...
        if comm.is_main_process():
            ret.append(
                hooks.BestCheckpointer(
                    cfg.TEST.EVAL_PERIOD, self.checkpointer, "bbox/AP"
                )
            )
